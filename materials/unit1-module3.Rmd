...
The idea is appealing, but since this is a course on "Spatial data analysis with R", I believe we should still learn base `R`, because many spatial packages are designed with base `R` in mind. Although `tidyverse` compliance/compatibility among spatial packages is growing, in practice many of the examples you will find for them use base `R` syntax, particularly the flagship package for raster-based analyses, [`terra`](https://rspatial.org/pkg/1-introduction.html), which is the much faster replacement for the workhorse `raster` package. 
...
- Create a new folder called "notebooks" in your class project (e.g. my `ls320`), as a top level folder. You can keep using this folder for all class notes using markdown.
- Open a new Rmarkdown file. Save it into the notebooks folder, calling it `sda320_unit1module3_practice.Rmd`. Adjust the title as needed, and delete the text and chunks below. Keep one chunk, but delete the code in it, as a starting point for code
...
#### A word on factors
One thing to point out about `data.frame` is that it used to, prior to R4.0, default to treating a character variable as a **factor**. Here is a [definition](https://www.stat.berkeley.edu/~s133/factors.html):
...
## Set-up
Make sure you are working in the main branch of your project. Create a new vignette named "unit1_module3.Rmd" (hint: you can use `usethis::use_vignette("unit1_module3")`. You will use this to document the tasks you undertake for this assignment.
...
#' XX function
#' Make sure you write a proper function help
#' by referencing back to Module 1
#' 
...
So you have to adapt the function by 1) adding a second argument for y, 2) changing the operations performed within the conditional statements, 3) the values of the arguments that "fun" responds to, and 4) the function name. 

Remember to run `devtools::document()` to update the package once you finish the new function.
...
1. Create the following vectors: 
    ...
    - IMPORTANT: In making `v1, v2, v3`, you will use the `set.seed` function so that you can get the **exact same** results each time you run your code to make them. **Use a seed value of 1 (`set.seed(1)` before the creation of `v1, v2, v3`).**
...
3. Create a `data.frame` (named `DF`) by bindings the columns of `v1`, `v2`, `v3`, and `v4`. 
    ...
    -  **Note that you need to answer these questions in your vignette "unit1_module3".**
...
4. Demonstrate the use of your new function: 
    - Use `my_calc` to sum and divide the following pairs of objects:
        ...
        - `m1`, `m1`
        - `m1`, `m2`
    - Capture the output of each operation in objects `d1` - `d5` for the divisions, and `s1` - `s5` for the sums. 
    - Using `my_calc` with `DF`:
        - Multiply row1, column1 by row1, column2
        - Multiply rows 1-10, column 1-3 by rows 1-10, columns 1-3
        - Divide the values in columns "a" and "b" by the values in column "c" of `DF` corresponding to the rows in "grp" that have values "E". In selecting the columns, use their names, rather than their index numbers, to select them (Hint: how to select specific rows conditionally in base R?).
    ...
    - Question: Can we use `my_calc` to `m2` and `m3`? And why?

5. Demonstrate the use of `*apply`:
    1. Combine `v1`, `m1`, and `m2` into a list `l`
    2. Write an `lapply` that applies `sum` to `l`. This one does not need an anonymous function 
    3. Now write an `lapply` that iterates over the elements of `l`, and uses your `my_calc` to multiply each element of `l` by `v2`. Some hints:
    ...
    - Do 2 and 3 but with `sapply` this time.
...
2. Has a DESCRIPTION and NAMESPACE file that shows that both your functions are exported. Fill out the DESCRIPTION file with your relevant details, a description of the package, etc. Also, use 3-digit version numbering. 0.1.0 is the initial version of your package. 0.1.X (where X means some integer in a sequence, 1 if this is your first minor update) is the updated version after a minor fix (a small bug corrected or documentation updated), 0.X.0 is an intermediate release (maybe you have added a new function or two), X.1.0 is a major release of a working version of the package. So for assignment 2, you should change the version number from 0.1.0 to 0.1.1.
...
7. Update vignette title and this line in the YAML header to `%\VignetteIndexEntry{Unit 1 Module 3 assignment 2}`. When you browse the vignettes of your package, you should see a page in your browser like this:

```{r out.width = "100%", fig.align='center', echo=FALSE}
include_graphics("fig/u1m3_a2.png") 
```

8. When complete, park the repo on a new side branch called "a2", and switch back to main branch for future work. Please check module 1 if you forget how to do this.
