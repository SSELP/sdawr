...
5.
```{r, eval=FALSE}
fs <- dir(system.file("extdata/", package = "sdadata"), pattern = "FAOSTAT", 
          full.names = TRUE)
...
1. 
```{r, eval = FALSE}
crop_ylds %>% filter(crop == "sorghum" & country == "TZA" & year >= 2000)
crop_ylds[crop_ylds$crop == "sorghum" & crop_ylds$country == "TZA" & 
            crop_ylds$year >= 2000, ]
```
...
summary(lm(yield ~ year, 
           data = crop_ylds %>% filter(crop == "maize" & country == "TZA")))
...
crop_ylds %>% filter(crop != "sorghum") %>% 
  ...
  nest() %>%
  mutate(prod_ha_lm = map(data, ~lm(yield ~ year, data = .x)),
         tidied = map(prod_ha_lm, broom::tidy)) %>%
  unnest(tidied)
...
crop_ylds %>%
  filter(crop != "sorghum") %>%
  group_by(crop, country) %>%
  do(broom::tidy(lm(yield ~ year, data = .)))
```
...
2.
```{r, eval = FALSE}
crop_ylds %>% filter(crop == "wheat" & country == "TZA") %>% 
  plot(harv_area ~ year, data = ., pch = 16, col = "blue", 
       xlab = "", ylab = "Harvested area (ha)", 
       main = "Tanzania wheat (1961-2024)")
crop_ylds %>% filter(crop == "wheat" & country == "TZA") %>% 
...
crop_ylds %>% filter(crop == "wheat" & country == "TZA") %>% 
  ...
  xlab("") + ylab("Harvested area (ha)") +  
  ggtitle("Tanzania wheat (1961-2024)")
crop_ylds %>% filter(crop == "wheat" & country == "TZA") %>% 
...
crop_ylds %>% filter(crop == "wheat" & country == "TZA") %>% 
  ...
  xlab("") + ylab("Harvested area (ha)") +  
  ggtitle("Tanzanian wheat (1961-2024)")
```
...
crop_ylds %>% filter(crop == "wheat" & country == "ZMB") %>% 
  ...
  xlab("Harvested area (ha)") + ggtitle("Zambian Wheat (1961-2024)")

# hist
# with dplyr
crop_ylds %>% filter(crop == "wheat" & country == "ZMB") %>% pull(harv_area) %>% 
  hist(., main = "Zambian Wheat (1961-2024)", xlab = "Harvested area (ha)", 
       col = "blue")

# with base subsetting
hist(crop_ylds$harv_area[crop_ylds$crop == "wheat" & 
                           crop_ylds$country == "ZMB"], 
     main = "Zambian Wheat (1961-2024)", xlab = "Harvested area (ha)", 
     col = "blue")
```

7. 
```{r, eval = FALSE}
crop_ylds %>% filter(country == "TZA") %>% 
