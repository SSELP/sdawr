...
load_quietly("tidyterra") # the package to plot spatRaster in a tidy way.
...
ggplot() +
    geom_spatraster(data = raintotcut) +
    # add a title
    ggtitle("Total Rainfall") +
    # Manually set the colors
    scale_fill_manual(
        name = "",
        values = c("tan", "yellow3", "green4"),
...
# Note that the resultant raintotrcl has continuous values
raintotrcl <- classify(x = precip, rcl = rclmat, include.lowest = TRUE)

# Convert classes to a factor with desired labels/order
raintotrcl <- as.factor(raintotrcl)
levels(raintotrcl) <- data.frame(
  value = 1:3,
  label = c("Low", "Intermediate", "High"))

ggplot() +
    geom_spatraster(data = raintotrcl) +
    # add a title
    ggtitle("Total Rainfall") +
    # Manually set the colors
    scale_fill_manual(
...
This approach requires slightly more work (e.g. we typically need to convert the resultant raster into factor), but there are cases where you might need to use it, e.g. to reclassify another categorical raster using non-sequential categories. 
...
## Vignette tasks
1. Create a subset of `districts` (of course you have to read `districts`) by extracting districts 40, 95, 73, 145. Call it `districts_ss`. Use the extent of `districts_ss` (`ext(districts_ss)`) and crs of `districts_ss` to define the extent of a new raster `r`, which should have a resolution of 0.1&deg;. Use this `r` as a template for creating two new rasters, `rsamp` and `rrandn`. `rsamp` should be filled with randomly selected integers ranging between 10 and 50. `rrandn` should be filled with random numbers drawn from a normal distribution (`rnorm`) that has a mean of 30 and standard deviation of 5. Use **a seed of 1** in `set.seed` for both rasters. Stack `rsamp` and `rrandn` (name the stack `s`), mask that by `districts_ss`, and plot `s` using `plot_noaxes` (This function has been included in package `sdadata`, so you need to load it first). (Ref: Chunks 1, 3, 4, 16 in Unit2-Module2a)
...
5. Use `geodata`'s `worldclim_country` function to grab WorldClim's mean temperature ("tavg") dataset at a resolution of 2.5 (note this is not degrees, but minutes of a degree), and download it to somewhere on your local disk. That will give a `SpatRaster` with 12 layers, with each layer representing the average monthly temperature for each grid cell on the planet. Calculate the annual mean temperature for each cell, and then mask the result using `districts` to get your final raster, `tzatmean`. Plot the result. (Ref: Chunk 17, 35)

6. Classify the temperature data into three categories, low, medium, and high, using <20&deg;, 20-24&deg;, and >24&deg; as the break points for determining the classes.  Use the `classify` function with a reclassification matrix, which you should do like this:

    ```{r, eval = FALSE}
    trng <- global(tzatmean, range, na.rm = TRUE)
    cuts <- c(floor(trng[1, 1]), 20, 24, ceiling(trng[1, 2]))
    ```

    Here `global` is helping to find the values of tmin and tmax, which respectively define the lower bound of the "low" class and the upper bound of the "high" class. What are the functions `floor` and `ceiling` doing (**answer this in your vignette**)? Call the reclassified temperature raster `tzatclass`. Make the map using `geom_spatraster` with a categorical legend (using `scale_fill_manual`), and using the colors "blue", "yellow2", and "red" for the three classes. (Ref: Chunk 26, 38)

7. Recreate the `precip` layer (`precip <- bioclimz[["bio12"]]`, see the beginning chunk and chunk37 in unit2-module2b), then calculate the mean precipitation within each temperature zone defined by `tzatclass` (hint: you will need to resample `precip` using function `resample`). Call the resulting matrix `z`. Map the mean zonal precipitation values in `z` onto each temperature zone (using the `subst` function with `tzatclass` as the target; remember that `zonal` returns a matrix, and that `subst` requires equal length vector for the "from" and "to" values. Call the new raster `tzaprecz`, and then plot it using `geom_spatraster`, with a custom legend (as done in Task 6), using the rounded zonal precipitation values (rounded) as the legend labels (`labels = round(z$bio12)` in `scale_fill_manual`). Use colors "yellow2", "green3", and "blue" for the three classes. (Ref: Chunks 32, 33, 38)

8. Use `geodata::elevation_30s` again to download the elevation raster for Tanzania (call it `dem`). Aggregate it with `fact = 5` and call the result `dem5`. Use `terrain` to calculate aspect from `dem5` (call it `aspect`), selecting degrees as the output value. Then find all west-facing aspects (aspects >247.5 and <292.5), and all east facing aspects (>67.5 and <112.5), making new rasters respectively named `west` and `east`, e.g. `west <- aspect > 247.5 & aspect < 292.5)`. Stack these together with `aspect` and make a three-panel plot with `plot_noaxes` with titles "Aspect", "West", and "East". (Ref: Chunks 37, 42)
...
9. Using a random seed of 1, create two random samples of 100 each. The first one should be collected from within the west-facing cells (i.e. only be drawn from cells in `west` that have a cell of one), and the second from east-facing cells. To do this, set the cells equal to 0 in `east` and `west` to NA (e.g. `west[west == 0] <- NA`). Once you have collected those, convert the resulting objects to `sf`, and use those two sets of points to extract temperature values from `tzatmean` into a tibble `temp_stats`, which is going to look this: 
    ...
    temp_stats <- bind_rows(
        tibble(temp = terra::extract(tzatmean, westpts)[, 2], dat = "West"), 
        tibble(temp = terra::extract(tzatmean, eastpts)[, 2], dat = "East")) %>% 
        na.omit()
    ```

    Then use `temp_stats` with `ggplot` to make side-by-side boxplots to compare the distributions of west and east facing temperatures, modeled on the example in Chunk 40 #4. (Ref: Chunks 37, 40, 41)

10. **Extra credit worth (5 points).** Extract the centroids from each district in `districts` (call it `dcent`), and reproject the points to Albers, using the `st_crs(roads)` (You have to load `roads` first). Reproject `tzatmean` to Albers also, making the new resolution (5 km, i.e. 5000 m), using bilinear interpolation (call it `tzatmeanalb`). Then use `dcent` to extract the temperature values from `tzatmeanalb` (add the values to `dcent` as a new variable "temp" using `mutate`). Use `gstat` to create an IDW model (call it `idw`). To make the IDW work, which isn't `sf` compliant, some extra work will be required, as shown below (this is the step needed after the extract of temperature values)
...
1. You should increment your package version number by 1 on the lowest number (e.g. from 0.1.3 to 0.1.4) in the DESCRIPTION;
2. Revise the setting chunk in the beginning of your vignette to:
    ...
    to avoid unnecessary warning and message and set global values for figure height and width.

3. Update vignette title and this line in the YAML header to `%\VignetteIndexEntry{Unit 2 Module 2 assignment 5}`. When you browse the vignettes of your package, you should see a page in your browser like this:

```{r out.width = "100%", fig.align='center', echo=FALSE}
include_graphics("fig/u2m2_a5.png")
```

4. Your submission should be on a new side branch "a5";
