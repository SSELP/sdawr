...
In #2, we first pull out of `species_env` a vector of the distinct district IDs. Because `species` contains a few bad coordinates that fall outside of Zambia, and there had `NA` values, we inserted `drop_na` into the pipeline to get rid of those. We then used `speciesdistid` to filter out the districts the farmers live in from `districts`, and used the result to extract from `precip` the mean rainfall from the pixels of `precip` falling within each selected district. When using `terra::extract` with polygons, if you don't pass a value to the "fun" argument, a list is returned, in which each element contains the extracted pixel values corresponding to a particular polyon. If you give "fun" an argument (e.g. `mean`, as in this case), you get back a `data.frame` containing the resulting statistic(s) for each polygon. 
