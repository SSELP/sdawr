...
- Run `x1 + x2`, `x1 * c(x2, 1)`, observe what is going on.
- Run `c(x1, x2)`, `rbind(x1, x2)`, and `cbind(x1, c(x2, 1))`.
...
- **Task for you**: create your own version of `x` about yourself. The role could be anything: e.g., student, football player, cat lover, meteorologist, geographer, etc. Then update the class of `x` to include your name, just like what I did.
- Print your `x` out.
...
- Customize the order of the levels.

```{r}
#| echo: true
#| eval: false
x_f2 <- factor(c("John", "Chris", "Rachel"), levels = c("John", "Chris", "Rachel"))
x_f2
x_f
```
...
- Remap the values (usually not necessary).

```{r}
#| echo: true
#| eval: false
x_f3 <- factor(c("John", "Chris", "Rachel"), 
               levels = c("John", "Chris", "Rachel"), # map from
               labels = c("Male", "Male", "Female")) # map to
x_f3
levels(x_f3)
labels(x_f3)
```
...
- **your task:** create a function `plot` for yourself class. Let the function do something that you want.
